...
# Tutorial: https://stackoverflow.com/questions/37239970/connect-to-mysql-server-without-sudo
...
apk add php81 php81-mysqli phpmyadmin php81-apache2
...
# If "mysql -u root -p" command shows connection error, then become super user using command "sudo su"
mysql -u root -p= -Bse "RENAME USER 'root'@'localhost' TO 'root'@'%';GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' WITH GRANT OPTION;FLUSH PRIVILEGES;"
# mysql -u root -p= -Bse "DROP USER 'root'@'localhost';CREATE USER 'root'@'%' IDENTIFIED BY '';GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' WITH GRANT OPTION;FLUSH PRIVILEGES;"
