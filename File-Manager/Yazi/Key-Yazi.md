...
* Press `<Esc>` or `Ctrl + c` or `q` to escape from various situation.

* Press `q` to quit, `<F1>` or `~` to open the help menu.

* Yazi supports vim motions
  * Start visual selection mode with `v`. Press `G` to mark upto last file, press `5j` to mark upto 5th file.
...
* `g ⇒ <Space>` : Jump interactively
  * Press `<Tab>` key to fill from auto-completion
  * Press `Down-Arrow` or `Ctrl + n` to scroll down
  * Press `Up-Arrow` or `Ctrl + p` to scroll up
...
* `s` : Search files by name using [fd](https://github.com/sharkdp/fd)
  * Press `Ctrl + s` or `<Esc>` to return from filtered files list to all files list.
...
* `w` => `show` : Show task manager
  ...
  * To see log details, press `Enter`, then press `q` to quit from log details screen.
  * Press `<Esc>` to close task manager window.
