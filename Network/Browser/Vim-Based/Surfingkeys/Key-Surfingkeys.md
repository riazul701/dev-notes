...
# Keybindings From `?` Help

* Press `<Esc>` to exit from various situation.
...
* `<Alt-i>` : Enter PassThrough mode to temporarily suppress SurfingKeys
  * Press `Esc` to leave PassThrough mode and return to normal Surfingkeys behavior.
  * In Surfingkeys, PassThrough mode temporarily disables all of Surfingkeys' keyboard shortcuts so that every key press is sent directly to the website instead.
...
* `[[` : Click on the previous link on current page
  * It tries to find and click a "previous" navigation link within the current webpage's pagination.

* `]]` : Click on the next link on current page
  * It tries to find and click a "next" navigation link within the current webpage's pagination.
...
* `0` : Scroll all the way to the left
  * Example Page: [rustdesk/rustdesk GitHub](https://github.com/rustdesk/rustdesk)
...
* `;w` : Focus top window
  ...
  * Example Page: [HTML Iframes - W3Schools](https://www.w3schools.com/html/html_iframe.asp)
